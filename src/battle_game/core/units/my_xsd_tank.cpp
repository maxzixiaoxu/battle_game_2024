...
xsdTank::xsdTank(GameCore *game_core, uint32_t id, uint32_t player_id)
...
void xsdTank::TankMove(float move_speed, float rotate_angular_speed) {
...
bool xsdTank::IsHit(glm::vec2 position) const {
...
const char *xsdTank::UnitName() const {
  return "XSD Tank";
}

const char *xsdTank::Author() const {
  return "XSD";
}
}  // namespace battle_game::unit
